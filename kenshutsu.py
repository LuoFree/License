...
if __name__ == '__main__':
    ...
    for image_name in os.listdir(root):
            ...
            if c == 2 or c == 5:
                ...
                if result:
                    ...
                    plates.append((x1,y1,x2,y2,plate,
                                   int(x11+x1),int(y11+y1),
                                   int(x22+x1),int(y22+y1)))
        if not plates:
            print("未检测到车辆，尝试检测车牌")
            for plate,(x11,y11,x22,y22) in read_plate(image):
                plates.append((None,None,None,None,plate,
        ...
        print(image_name)
        # cv2.imshow('a', image) # 如果要边检测边查看检测结果图，就把这句和下面一句话打开
        # cv2.waitKey()
        # for box in boxes:
        #     print("检测框内容：", box)
        #     x1, y1, x2, y2, the, c = box
        #     print(f"类别索引 c={c}, 坐标=({x1},{y1},{x2},{y2})")
